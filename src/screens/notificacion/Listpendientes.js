  ...
  const fetchData = async (page) => {
    ...
    if (networkStatus.connected) {
      ...
      if (sessionDatadirectory !== undefined && sessionDatadirectory > 0) {
        result = await http.get(process.env.REACT_APP_PATH + '/notificacion/' + page + '/' + state.rowsPerPage + '?estado=1&uid=' + sessionDatadirectory);
      } else {
            ...
            <Table stickyHeader aria-label="sticky table" className='mt-5'>
              <TableHead>
                <TableRow>
                  ...
                  <StyledTableCell style={{ minWidth: '10%', maxWidth: '10%' }} className='bg-gore border-table'>Emisor
                  </StyledTableCell>
                  <StyledTableCell style={{ minWidth: '70%', maxWidth: '70%' }} className='bg-gore border-table'>Asunto
                  </StyledTableCell>
