...
import { Button, Checkbox, Fab, styled, Table, TableCell, TextField, TablePagination, TableHead, TableBody, TableRow, TableContainer, Toolbar, Grid, CardContent, Card, Typography } from '@mui/material';
...
const List = () => {
  ...
  const fetchData = async (page) => {
    ...
    if (networkStatus.connected) {
      ...
      const result = await http.get(process.env.REACT_APP_PATH + '/notificacion/all/' + page + '/' + state.rowsPerPage);
  ...
  const editarNotificacionOnClick = () => {
    navigate('/notificacion/' + selected[0] + '/edit');
  };
          ...
          <Toolbar className="Toolbar-table" direction="row" >
            <Grid container spacing={2} sx={{ marginY: '1em !important' }}>
              <Grid item xs={12} sm={12} md={3}>
                <Button sx={{ width: '100%', fontWeight: 'bold' }} disabled={!selected.length} startIcon={<InsertDriveFile />} onClick={editarNotificacionOnClick} variant="contained" color="primary">Editar</Button>
              </Grid>
              <Grid item xs={12} sm={12} md={3}>
                <Button sx={{ width: '100%', fontWeight: 'bold' }} disabled={!selected.length} startIcon={<InsertDriveFile />} onClick={verNotificacionOnClick} variant="contained" color="primary">Ver Notificación</Button>
              </Grid>
              <Grid item xs={12} sm={12} md={3}>
                <Button sx={{ width: '100%', fontWeight: 'bold' }} disabled={!selected.length} onClick={onClickDestacar} startIcon={<Star />} variant="contained" color="primary">Destacar</Button>
              </Grid>
              <Grid item xs={12} sm={12} md={3}>
            ...
            <Table stickyHeader aria-label="sticky table" className='mt-5'>
              <TableHead>
                <TableRow>
                  ...
                  <StyledTableCell style={{ minWidth: '20%', maxWidth: '20%' }} className='bg-gore border-table'>Remitente
                  </StyledTableCell>
                  <StyledTableCell style={{ minWidth: '65%', maxWidth: '65%' }} className='bg-gore border-table'>Asunto
                  </StyledTableCell>
                  <StyledTableCell style={{ minWidth: '20%', maxWidth: '20%' }} className='bg-gore border-table'>Destinatario
                  </StyledTableCell>
                  ...
                  .map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell style={{ minWidth: '65%', maxWidth: '65%' }}>
