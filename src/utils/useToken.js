    ...
    if (code) {
      ...
      http.post('https://web.regionancash.gob.pe/api/auth/token', code, () => { return { 'Content-Type': '*/*' } }).then((data) => {
        if (data.error) {
          ...
          let tokenString = JSON.stringify(data.token);

          // Eliminar las comillas que rodean la cadena del token
          let token = tokenString.slice(1, -1);

          // Decodificar el token en Base64 y convertirlo a objeto JSON
          const [headerBase64, payloadBase64] = token.split('.'); // Divide el token en partes
          const header = JSON.parse(atob(headerBase64));
          const payload = JSON.parse(atob(payloadBase64));

          localStorage.setItem('directory', payload.directory);
    ...
    {!profile ? <form>
      ...
      <div style={{ padding: 20, textAlign: 'center' }}>
        <TabPanel value={value} index={0} >
          ...
          <Button onClick={loginOnClick} style={{
            ...styles.base, textAlign: 'center',
            justifyContent: "center", backgroundColor: '#b71212'
